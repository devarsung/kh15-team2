...
$(function() {
	$("[name=boardContent]").summernote({
		...
		placeholder: "타인에 대한 무분별한 비방 시 예고 없이 삭제될 수 있습니다",//도움말
		//메뉴(toolbar) 설정
		toolbar: [
			//['메뉴명', ['버튼명', '버튼명', ...]]
		...
		callbacks: {
			onImageUpload: function(files) {
				...
				//  - 상황상 form을 쓸 수가 없으므로 ajax를 써야한다
				...
				$.ajax({
					processData: false,//파일 업로드를 위해 반드시 필요한 설정
					contentType: false,//파일 업로드를 위해 반드시 필요한 설정
					url: "/rest/board/uploads",
					method: "post",
					data: form,
					success: function(response) {//첨부파일 번호들 List<Integer>
...
$(function(){
	...
	function checkInputState() {
		...
		var isEmpty = titleEmpty && contentEmpty;//제목 비어있음 && 내용 비어있음
