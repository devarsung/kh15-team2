$(function(){       
	...
	$(".btn-send-cert").click(function(){
		...
		$.ajax({
			...
			//beforeSend - 전송 전 자동 실행되는 콜백함수 설정 영역
			//error - 통신 오류 발생 시 자동 실행되는 콜백함수 설정 영역(보내다가 에러났을 때)
			//complete - 오류와 관계 없이 통신 완료 시 자동 실행 되는 콜백함수 설정 영역(보냈는지 안보냈는지는 모르겠지만 통신이 끝난 시점)
			beforeSend: function() {
				$(".btn-send-cert").prop("disabled", true);
				$(".btn-send-cert").find("span").text("이메일 발송중");
				$(".btn-send-cert").find("i").removeClass("fa-paper-plane").addClass("fa-spinner fa-spin");
			},
			complete: function() {
		...
		$.ajax({
			...
			success: function(response) {//response는 true/false 중 하나
				...
				if(response == true) {
					...
					$(".btn-confirm-cert").prop("disabled", true);//숨겨져있어도 disabled 처리
